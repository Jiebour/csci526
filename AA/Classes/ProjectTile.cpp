...
//cageTower
CageProjectTile* CageProjectTile::create(cocos2d:: CCPoint pos){
    CageProjectTile* m = new CageProjectTile;
    if(m && m->initWithTargetPos(pos)){
        m->autorelease();
        return m;
    }
...
bool CageProjectTile::initWithTargetPos(cocos2d::CCPoint pos){
    bool bRet = false;
    do
    {
        CC_BREAK_IF(!initWithMyFile());
...
bool CageProjectTile::initWithMyFile(){
    init();
...
void CageProjectTile::update(float dt){
    //GameMediator* gm = GameMediator::sharedMediator();
    ...
    CCARRAY_FOREACH(gm->getTargets(), temp){
        ...
        float dis =  ccpDistance(this->getPosition(), target->getPosition());
        if(dis<=getRange() ){
            target->changeSpeedCage();
            //target->removeSelf();
           // CCLOG("yunxing..... tile pos:%s","yesssss");
...
//YummyTower
YummyProjectTile* YummyProjectTile::create(cocos2d:: CCPoint pos){
    YummyProjectTile* m = new YummyProjectTile;
    if(m && m->initWithTargetPos(pos)){
        m->autorelease();
        return m;
    }
...
bool YummyProjectTile::initWithTargetPos(cocos2d::CCPoint pos){
    bool bRet = false;
    do
    {
        CC_BREAK_IF(!initWithMyFile());
...
bool YummyProjectTile::initWithMyFile(){
    init();
...
void YummyProjectTile::update(float dt){
    //GameMediator* gm = GameMediator::sharedMediator();
    ...
    CCARRAY_FOREACH(gm->getTargets(), temp){
        Enemy* target = (Enemy*)temp;
        float dis =  ccpDistance(this->getPosition(), target->getPosition());
        if(dis<=getRange()&&target->eating==0){
            target->changeSpeedYummy();
            //target->removeSelf();
            // CCLOG("yunxing..... tile pos:%s","yesssss");

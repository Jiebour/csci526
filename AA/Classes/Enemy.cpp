...
bool Enemy::initWithMem(const char* filename, int hp, float speed,int gift,CCPoint pos){
	...
	do
	{
        ...
        frame0=CCSpriteFrame::createWithTexture(heroTexture, cocos2d::CCRectMake(50*0,77.25*1,50,77.25));
        frame1=CCSpriteFrame::createWithTexture(heroTexture, cocos2d::CCRectMake(50*1,77.25*1,50,77.25));
        frame2=CCSpriteFrame::createWithTexture(heroTexture, cocos2d::CCRectMake(50*2,77.25*1,50,77.25));
        frame3=CCSpriteFrame::createWithTexture(heroTexture, cocos2d::CCRectMake(50*3,77.25*1,50,77.25));
		...
		x =-20;// pos.x/2;
		y=  pos.y+10 ;
		startPos = ccp(x,y);//定义坐标
        //CCLOG("bear point tile pos:%d,%d",x,y);
...
CCAnimation* Enemy::createAnimationByDirection(HeroDirection direction){
    ...
    //第二个参数表示显示区域的x,y,width,height
    int tmp=direction;
    if (tmp==3 || tmp==2) tmp=5-tmp;
    frame0=CCSpriteFrame::createWithTexture(heroTexture, cocos2d::CCRectMake(50*0,77.25*direction,50,77.25));
    frame1=CCSpriteFrame::createWithTexture(heroTexture, cocos2d::CCRectMake(50*1,77.25*direction,50,77.25));
    frame2=CCSpriteFrame::createWithTexture(heroTexture, cocos2d::CCRectMake(50*2,77.25*direction,50,77.25));
    frame3=CCSpriteFrame::createWithTexture(heroTexture, cocos2d::CCRectMake(50*3,77.25*direction,50,77.25));
    //CCMutableArray<CCSpriteFrame*> *animFramess=new CCMutableArray<CCSpriteFrame*>(4);
...
//cage
int Enemy::changeSpeedCage(float time){
    if(getHP() >500)
        return 1;
    removeSelf();
    return 0;
...
void Enemy::changeSpeedYummy(float time){
    CCLOG("xi zhu le");
    speed=0.01f*speed;
    eating=1;
...
FastRedEnemy* FastRedEnemy::create(cocos2d::CCPoint pos){
   // CCSprite *sp =CCSprite::createWithSpriteFrameName("Obss9_9-41.png");
	...
	return FastRedEnemy::create("hero_phy.png", 400, 64.0f,8,pos);
}
...
StrongGreenEnemy* StrongGreenEnemy::create(cocos2d::CCPoint pos){
	return StrongGreenEnemy::create("hero_fast.png", 600, 32.0f,5,pos);//
}

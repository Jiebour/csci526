...
void MainLayer::createTileMap(){
  ...
  //  CCSize wSize = CCDirector::sharedDirector()->getWinSize();
   ...
   // gameMap->setScale(scale);
    ...
    CCLOG("way tile pos:%d,%d",maxTileHeight,maxTileWidth);
	for(int i = 0; i < maxTileWidth; i++){
		for(int j = 0; j < maxTileHeight; j++){
            ...
            if (dic!=NULL && dic->valueForKey("way")->intValue()==2){
                td->setIsUsed(true);
            }
            if  (dic!=NULL&&dic->valueForKey("way")->intValue()==1) {
             ...
             /*   if(dic->valueForKey("attr")->intValue()==0){
                    td->setIsDonut(true);
                    Tower* target = NULL;
                    target = CageTower::create();
                    target->setPosition(positionForTileCoord(td->getPosition()));
                    
                    this->addChild(target, 5);
                    GameMediator* m = GameMediator::sharedMediator();
                    m->getTowers()->addObject(target);
                    //CCLOG("yunxinglemeiyoua :%s", "meiyou");
                }*/
            }
...
CCPoint MainLayer::tileCoordForPosition(cocos2d::CCPoint position){
	...
	if(position.y < TILE_HEIGHT * 2){
		...
		y = (gameMap->getContentSize().height-position.y ) / TILE_HEIGHT;
	}
...
void MainLayer::addTower(CCPoint pos, int towerTag){
	...
	if(towerTag==5)
    {
        ...
        if (gameHUD->getResources()>=Tower5Cost)
        {
            target=CageTower::create();
            gameHUD->updateResources(-Tower5Cost);
        }
        else return;
    ...
    if (towerTag==6)
    {
        ...
        if (gameHUD->getResources()>=Tower6Cost)
        {
            target=YummyTower::create();
            gameHUD->updateResources(-Tower6Cost);
        }
        else return;
    ...
    if(canBuildOnTilePosition(pos)){
		switch(towerTag){
            case 1:
                if(gameHUD->getResources() >= Tower1Cost){
                    target = MachineGunTower::create();
                    gameHUD->updateResources(-Tower1Cost);
                }else{
                    return;
                }
                ...
                break;
            case 2:
                if(gameHUD->getResources() >= Tower2Cost){
                    target = FreezeTower::create();
                    gameHUD->updateResources(-Tower2Cost);
                }else{
                    return;
                }
                break;
            case 3:
                if(gameHUD->getResources() >= Tower3Cost){
                    target = CannonTower::create();
                    gameHUD->updateResources(-Tower3Cost);
                }else{
                    return;
                }
                break;
            case 4:
                if(gameHUD->getResources() >= Tower4Cost){
                    target = MutilTower::create();
                    gameHUD->updateResources(-Tower4Cost);
                }else{
                    return;
                }
                break;
...
void MainLayer::addWaves(){
	...
	Wave* wave = Wave::create(3.0f, 0, 20);
	m->getWaves()->addObject(wave);
	wave = NULL;
    
	wave = Wave::create(1.5f, 15, 15);
	m->getWaves()->addObject(wave);
	wave = NULL;
    
	wave = Wave::create(0.8f, 25, 15);
	m->getWaves()->addObject(wave);
	wave = NULL;
    
	wave = Wave::create(0.5f, 30, 10);
	m->getWaves()->addObject(wave);
	wave = NULL;
    
	wave = Wave::create(0.3f, 50, 0);
	m->getWaves()->addObject(wave);
	wave = NULL;

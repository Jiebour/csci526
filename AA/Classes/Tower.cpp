...
bool Tower::ccTouchBegan(cocos2d::CCTouch *pTouch, cocos2d::CCEvent *pEvent){
	...
	if(sprite->boundingBox().containsPoint(touchLocation)){
		if(isShowing == false&&!_cage&&!_yummy){
			isShowing = true;
			show();
		}else{
			isShowing = false;
    ...
    if (isShowing)
    {
        ...
        if(isShowing && sprite3->boundingBox().containsPoint(touchLocation)){
            ...
            if (GameHUD::sharedHUD()->getResources()<getMoney()) {
                ...
                moneyEnough = true;
            if (moneyEnough) {
                if (_level<TowerMaxLevel&&!_cage&&!_yummy) {
                    _level++;
                    levelUp();
                }
            }
...
//**add cageTower
CageTower* CageTower::create(const char *pszFilename, int range){
    CageTower* t = new CageTower;
    if(t && t->initWithFileAndRange(pszFilename, range)){
        t->autorelease();
        return t;
    }
...
CageTower* CageTower::create(CCSpriteFrame* frame, int range){
    CageTower* t = new CageTower;
    if(t && t->initWithSpriteFrame(frame, range)){
        t->autorelease();
        return t;
...
CageTower* CageTower::create(){
    return //FreezeTower::create("FreezeTurret.png", 150);
    CageTower::create(Tower5_1, T5Range);
    //_cage = true;
}
bool CageTower::initWithSpriteFrame(cocos2d::CCSpriteFrame *frame, int range){
    bool bret = false;
    do {
        CC_BREAK_IF(!Tower::initWithSpriteFrame(frame, range));
        setMoney(Tower5Cost);
        ...
        _cage = true;
        killed=0;
        schedule(schedule_selector(CageTower::fire), T5Interval);
        this->schedule(schedule_selector(Tower::towerLogic), 0.1f);
        bret = true;
    } while (0);
    return bret;
}
bool CageTower::initWithFileAndRange(const char *pszFilename, int range){
    bool bRet = false;
    do
    {
        CC_BREAK_IF(!Tower::initWithFileAndRange(pszFilename, range));
...
void CageTower::fire(float dt){
    if(this->getTarget() != NULL){
        ...
        CageProjectTile* ProjectTile = CageProjectTile::create(offscreenPoint);
        ProjectTile->setPosition(this->getPosition());
        ProjectTile->setRotation(this->getRotation());
        ProjectTile->setDamage(_damge);
        ProjectTile->setMaxDamge(_maxDamge);
        ...
        m->getGameLayer()->addChild(ProjectTile);
        this->killed++;
        if (this->killed>MAXKILL_5)GameMediator::sharedMediator()->getGameLayer()->removeTower(this);
    }
}

//add YummyTower:

YummyTower* YummyTower::create(const char *pszFilename, int range){
    YummyTower* t = new YummyTower;
    if(t && t->initWithFileAndRange(pszFilename, range)){
        t->autorelease();
        return t;
    }
...
YummyTower* YummyTower::create(CCSpriteFrame* frame, int range){
    YummyTower* t = new YummyTower;
    if(t && t->initWithSpriteFrame(frame, range)){
        t->autorelease();
        return t;
...
YummyTower* YummyTower::create(){
    return //FreezeTower::create("FreezeTurret.png", 150);
    YummyTower::create(Tower6_1, T6Range);
}
bool YummyTower::initWithSpriteFrame(cocos2d::CCSpriteFrame *frame, int range){
    bool bret = false;
    do {
        CC_BREAK_IF(!Tower::initWithSpriteFrame(frame, range));
        setMoney(Tower6Cost);
        ...
        _yummy=true;
        killed=0;
        schedule(schedule_selector(YummyTower::fire), T6Interval);
        this->schedule(schedule_selector(Tower::towerLogic), 0.1f);
        bret = true;
    } while (0);
    return bret;
}
bool YummyTower::initWithFileAndRange(const char *pszFilename, int range){
    bool bRet = false;
    do
    {
        CC_BREAK_IF(!Tower::initWithFileAndRange(pszFilename, range));
...
void YummyTower::fire(float dt){
    ...
    CCARRAY_FOREACH(m->getTargets(), temp)
    {
        ...
        double curDistance = ccpDistance(this->getPosition(), enemy->getPosition());
        if(curDistance <=getRange() && enemy->eating==0){
            CocosDenshion::SimpleAudioEngine::sharedEngine()->playEffect("tower1.wav");
            enemy->changeSpeedYummy();
            this->killed++;
            CCLOG("sha le%d",this->killed);
            if (this->killed>MAXKILL_6)
            {
                GameMediator::sharedMediator()->getGameLayer()->removeTower(this);
                return;
            }
